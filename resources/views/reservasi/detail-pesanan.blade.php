        ...
        @if(isset($rescheduleInfo) && $rescheduleInfo && in_array($status ?? 'booking', ['booking', 'pembayaran', 'berhasil']))
            <div class="border-t-2 border-[#017249]/10 px-6 md:px-8 py-6">
                ...
                @if($rescheduleInfo['payment_status'] === 'payment_required')
                    <div class="flex items-start gap-3 bg-[#017249]/5 border border-[#017249]/20 rounded-xl p-4 mb-5">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-600 mt-0.5">Karena terdapat tambahan biaya, Anda perlu membayar
                ...
                @elseif($rescheduleInfo['payment_status'] === 'refund_due')
                    <div class="flex items-start gap-3 bg-[#017249]/5 border border-[#017249]/20 rounded-xl p-4 mb-5 flex-col sm:flex-row">
                        <div class="flex gap-3">
                            <svg class="w-5 h-5 text-[#017249] mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <div>
                                <p class="text-sm font-bold text-[#017249]">Pengajuan Refund</p>
                                @if(($status ?? 'booking') === 'booking')
                                    <p class="text-xs text-gray-600 mt-0.5">Pilihan baru Anda lebih murah (selisih <span class="font-bold text-[#017249]">{{ $rescheduleInfo['payable_display'] }}</span>). Silahkan klik "Continue Order" terlebih dahulu untuk mengkonfirmasi reschedule Anda.</p>
                                @else
                                    <p class="text-xs text-gray-600 mt-0.5">Reschedule telah dikonfirmasi! Anda berhak mendapatkan refund sebesar <span class="font-bold text-[#017249]">{{ $rescheduleInfo['payable_display'] }}</span>. Silahkan hubungi Admin 1 via WhatsApp untuk mencairkan dana Anda.</p>
                                @endif
                            </div>
                        </div>
                        @if(($status ?? 'booking') === 'berhasil')
                            <div class="mt-2 sm:mt-0 sm:ml-auto">
                                <a href="https://wa.me/6287735482327?text=Halo%20Admin%20Pineus%20Tilu,%20saya%20ingin%20mengajukan%20refund%20karena%20reschedule%20pesanan%20dengan%20kode%20{{ $token ?? '' }}." target="_blank" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-[#25D366] text-white text-xs font-bold rounded-lg hover:bg-[#1da851] transition-colors shadow-sm">
                ...
                @else
                    <div class="flex items-start gap-3 bg-[#017249]/5 border border-[#017249]/20 rounded-xl p-4 mb-5">
                        ...
                        <div>
                            ...
                            @if(($status ?? 'booking') === 'booking')
                                <p class="text-xs text-gray-600 mt-0.5">Harga booking baru sama dengan sebelumnya. Silahkan klik "Continue Order" terlebih dahulu untuk mengkonfirmasi reschedule Anda.</p>
                            @else
                                <p class="text-xs text-gray-600 mt-0.5">Harga booking baru sama dengan sebelumnya. Reschedule Anda telah dikonfirmasi otomatis.</p>
                            @endif
                        </div>
                    </div>
                @endif
