...
@php
    $normalizeLabel = function ($label) {
        ...
        // Normalize duplicated words like "Tent Tent"
        $label = preg_replace('/\b(\w+)(\s+\1)+\b/i', '$1', $label);

        // Normalize duplicated deck numbers: "Deck 1, 2, 1, 2" => "Deck 1, 2"
        if (preg_match('/^Deck\s+(.+?)(\s*\(.*\))?$/i', $label, $matches)) {
            $numbersPart = $matches[1] ?? '';
            $suffix = $matches[2] ?? '';

            $numbers = array_filter(array_map(function ($n) {
                return trim($n);
            }, preg_split('/\s*,\s*/', $numbersPart)));

            $uniqueNumbers = array_values(array_unique($numbers));
            $label = 'Deck ' . implode(', ', $uniqueNumbers) . $suffix;
        }
...
<section class="py-6 bg-white relative overflow-hidden">
    <div class="max-w-6xl mx-auto px-6 relative z-10">
        {{-- Main Card Container --}}
        <div class="bg-[#0b5a3e] border border-emerald-700/30 shadow-lg overflow-hidden text-white relative rounded-2xl">
            {{-- Decorative blur elements --}}
            <div class="absolute inset-0 opacity-10">
                <div class="absolute top-0 left-0 w-72 h-72 bg-white blur-3xl -translate-x-1/2 -translate-y-1/2"></div>
                <div class="absolute bottom-0 right-0 w-96 h-96 bg-white blur-3xl translate-x-1/2 translate-y-1/2"></div>
            </div>
            ...
            <div class="px-6 py-10 md:px-10 md:py-12 relative z-10">
                ...
                <div class="text-center mb-14" data-aos="fade-up" data-aos-duration="600">
                    <h2 class="text-2xl md:text-4xl font-extrabold text-white tracking-wide" style="font-family:'Bizon',ui-sans-serif,system-ui;">PRICE & CAPACITY</h2>
                </div>

                <div class="grid md:grid-cols-2 gap-6 lg:gap-8">
                    {{-- PRICE Card --}}
                    <div class="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/15 overflow-hidden hover:bg-white/10 transition-all duration-300" data-aos="fade-right" data-aos-duration="600">
                        {{-- Card Header --}}
                        <div class="px-6 py-4 bg-white/5 border-b border-white/10">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-emerald-400/20 rounded-2xl flex items-center justify-center">
                        ...
                        <div class="p-5 space-y-6">
                            @if(isset($prices) && count($prices))
                                @foreach(['weekday' => 'Weekdays', 'weekend' => 'Weekend', 'high_season' => 'High Season'] as $key => $label)
                                    @if(isset($prices[$key]) && count($prices[$key]))
                                        <div class="group">
                                            <button type="button"
                                                class="ft-toggle w-full text-left flex items-center justify-between px-4 py-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all duration-300 cursor-pointer"
                                                data-target="price-{{ $key }}" data-group="price" aria-expanded="false">
                                                <div class="flex items-center gap-3">
                                                    <span class="w-8 h-8 bg-emerald-400/20 rounded-2xl flex items-center justify-center">
                                                ...
                                                <svg class="w-5 h-5 text-white/70 ft-icon transition-transform duration-300" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            ...
                            @else
                                ...
                                <div class="group">
                                    <button type="button"
                                        class="ft-toggle w-full text-left flex items-center justify-between px-4 py-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all duration-300"
                                ...
                                <div class="group">
                                    <button type="button"
                                        ...
                                        <div class="flex items-center gap-3">
                                            <span class="w-8 h-8 bg-amber-400/20 rounded-2xl flex items-center justify-center">
                                ...
                                <div class="group">
                                    <button type="button"
                                        ...
                                        <div class="flex items-center gap-3">
                                            <span class="w-8 h-8 bg-rose-400/20 rounded-2xl flex items-center justify-center">
                    ...
                    {{-- CAPACITY Card --}}
                    <div class="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/15 overflow-hidden hover:bg-white/10 transition-all duration-300" data-aos="fade-left" data-aos-duration="600" data-aos-delay="100">
                        {{-- Card Header --}}
                        <div class="px-6 py-4 bg-white/5 border-b border-white/10">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-sky-400/20 rounded-2xl flex items-center justify-center">
                        ...
                        <div class="p-5 space-y-6">
                            ...
                            <div class="group">
                                <button type="button"
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="w-8 h-8 bg-sky-400/20 rounded-2xl flex items-center justify-center">
                                ...
                                <div id="cap-standard" class="bg-white/5 rounded-2xl p-4 mt-4 hidden border border-white/5">
                                    @if(($area->slug ?? null) === 'pineus-tilu-3-vip')
                                        {{-- Tent 6.3 Info --}}
                                        <div class="text-center mb-2">
                                            <p class="text-xs font-semibold text-white/80 mb-2 uppercase tracking-wide">Deck 1, 2, 3, 4, 5 (Tent 6.3)</p>
                                        </div>
                                    @endif

                                    <div class="flex items-center justify-center gap-6 mb-3">
                                        <div class="text-center">
                                            <div class="text-2xl font-bold text-sky-300">{{ $defaultPeople ?? 4 }}</div>
                                            <div class="text-xs text-white/60 uppercase tracking-wider">Standard</div>
                                        </div>
                                        <div class="text-white/40">—</div>
                                        <div class="text-center">
                                            <div class="text-2xl font-bold text-sky-300">{{ $maxPeople ?? 6 }}</div>
                                    ...
                                    @if(($area->slug ?? null) === 'pineus-tilu-3-vip')
                                        {{-- Tent 5.2 Info --}}
                                        <div class="text-center mb-3 pt-4 mt-4 border-t-2 border-white/25">
                                            <p class="text-xs font-semibold text-white/80 mb-2 uppercase tracking-wide">Deck 6, 7, 8, 9 (Tent 5.2)</p>
                                            <div class="flex items-center justify-center gap-6">
                                                <div class="text-center">
                                                    <div class="text-2xl font-bold text-sky-300">5</div>
                                                    <div class="text-xs text-white/60 uppercase tracking-wider">Standard</div>
                                                </div>
                                                <div class="text-white/40">—</div>
                                                <div class="text-center">
                                                    <div class="text-2xl font-bold text-sky-300">9</div>
                                                    <div class="text-xs text-white/60 uppercase tracking-wider">Maximum</div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <p class="text-xs text-white/70 italic text-center border-t-2 border-white/20 pt-4 mt-4">
                                        Guests above 2 years old are charged
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="group">
                                <button type="button"
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="w-8 h-8 bg-violet-400/20 rounded-2xl flex items-center justify-center">
                                            <svg class="w-4 h-4 text-violet-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                        </span>
                                        <span class="font-semibold">Additional Charges</span>
                                    </div>
                                    <svg class="w-5 h-5 text-white/70 ft-icon transition-transform duration-300" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path d="M6 9l6 6 6-6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                ...
                                <div id="cap-additional" class="bg-white/5 rounded-2xl p-4 mt-4 hidden border border-white/5">
                                    <div class="space-y-3">
                                        <div class="bg-white/5 rounded-2xl p-3 border border-white/10">
                                            ...
                                            <p class="text-xs text-white/70">Sleeping bag, breakfast, foam mattress + pillow</p>
                                        </div>
                                        ...
                                        <div class="bg-white/5 rounded-2xl p-3 border border-white/10">
                                            ...
                                            <p class="text-xs text-white/70">Breakfast only</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-3 mt-4">
                                @if(!str_contains($area->slug ?? '', 'cabin'))
                                    <div class="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-3">
                                        <div class="flex items-start gap-2">
                                            ...
                                            <p class="text-xs text-amber-200/90 leading-relaxed">
                                                <span class="font-semibold">Extra charges apply</span> for guests exceeding standard capacity
                                            </p>
                                        </div>
                                    </div>
                                @endif
                                
                                <div class="flex items-start gap-2 p-3 bg-white/5 rounded-2xl border border-white/10">
                                    ...
                                    <p class="text-xs text-white/70">
                                        For additional information, please contact the reservation admin.
                                    </p>
                                </div>
                            </div>
                        </div>
